...
var $_parse = {
  "360buy": function(doc, visa, kid, win_url){
    ...
    for(var i = 0; i < lis.length; i++){
      ...
      if(comment_counter_node == undefined)
        ...
        p.comment_counter = parseInt(comment_counter_node.textContent.match(/[0-9]+/)[0]);
  ...
  "newegg": function(doc, visa, kid, win_url){
    ...
    for(var i = 0; i < dls.length; i++){
      ...
      if(rank != undefined)
        p.comment_counter = parseInt(rank.textContent.match(/[0-9]+/)[0]);
      ...
      p.price = price_node.textContent.match(/[0-9\.]+/)[0];
  ...
  "tao3c": function(doc, visa, kid, win_url){
    ...
    for(var i = 0; i < lis.length; i++){
      ...
      var price_node = doc.querySelector('.pd4 span');
      if(price_node == undefined){
      ...
      httpGetjQuery(p.path, function(doc1){
        ...
        if(comment_node == undefined) 
          ...
          p.comment_counter = comment_node.textContent.match(/[0-9]+/)[0];
        out_p(p)
        saveOrUpdateProduct(p);
      });
    }
  ...
  "coo8": function(doc, visa, kid, win_url){
    ...
    for(var i = 0; i < lis.length; i++){
      var el = lis[i];
      var p = {category_id: kid};
      console.log("::::img_path", el.innerHTML)
    ...
    for(var i = 0; i < lis.length; i++){
      ...
      httpGetjQuery(p.path, function(doc1){
        ...
        if(comment_counter_node == undefined)
          ...
          comment_counter_node.textContent.match(/[0-9]+/)[0];
