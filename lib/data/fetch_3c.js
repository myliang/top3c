...
var $_parse = {
  "360buy": function($, visa, kid){
    ...
    if(p.length > 0){
      httpGetList(product_url[visa] + $(p[0]).attr('href'), visa, kid);
    }
  },
  "newegg": function($, visa, kid){
    $('.itemGrid itemCell dl.inner').each(function(i){
      var p = {category_id: kid};
      p.img_path = $('dt > a > img', this).attr('src');
      var p_a = $('dd.pdInfo .info a', this);
      p.name = p_a.text();
      p.path = p_a.attr('href');
      p.comment_counter = parseInt($('dd.rank a', this).text().match(/[0-9]+/g)[0]);
      p.price = $('dd.price .current .price', this).text().match(/[0-9\.]+/g)[0];
      p.kid = $('dd.btnArea a', this).first().attr('href').split('&')[1].split('=')[1] + "_" + visa;
      saveOrUpdateProduct(p);
    });
    var p = $('.pageNav .next');
    if(!p.hasClass('nextDisable')){
      httpGetList(p.attr('href'), visa, kid);
    }
  },
  "tao3c": function(){},
  "coo8": function(){},
...
function saveOrUpdateProduct(p){
  ...
  solrClient.add({type: 'Product', name_text: p.name, id: product._id.toString()}, function(err){
  //solrClient.add({class_name: 'Product', type: 'Product', name_text: p.name, id: "Product " + product._id.toString()}, function(err){
    if(err) throw err;
    solrClient.commit();
  });
  //console.log("::::", product._id.toString())
